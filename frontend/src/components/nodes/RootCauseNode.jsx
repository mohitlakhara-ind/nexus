...
import { Search, ArrowUp, Trash2, Edit3, Check, X, ChevronDown, ChevronUp, FileText } from 'lucide-react';
import NodeLinkBadge from '../NodeLinkBadge';

export default function RootCauseNode({ id, data }) {
  const [isEditing, setIsEditing] = useState(false);
  const [editLabel, setEditLabel] = useState(data.label || '');
  const [editDesc, setEditDesc] = useState(data.description || '');
  const [showDesc, setShowDesc] = useState(false);
  const votes = data.votes || 0;
  const accentColor = data.color || 'var(--node-root-border)';
  ...
  return (
    <div
      className="glass-card rounded-xl p-4 min-w-[240px] max-w-[300px] group transition-all duration-300 hover:scale-[1.02] border-l-4"
      style={{
        borderLeftColor: accentColor,
        boxShadow: `0 8px 32px -8px ${accentColor}55`,
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2.5">
          <div className="size-8 rounded-lg flex items-center justify-center" style={{ background: `${accentColor}20`, border: `1px solid ${accentColor}40` }}>
            <Search size={16} style={{ color: accentColor }} />
          </div>
          <span className="text-[10px] font-display font-bold uppercase tracking-[0.2em]" style={{ color: accentColor }}>Root Cause</span>
        </div>

        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={(e) => { e.stopPropagation(); setIsEditing(true); setShowDesc(true); }} className="p-1.5 hover:bg-main/5 rounded-lg text-muted hover:text-main transition-all">
            <Edit3 size={12} />
          </button>
          <button onClick={(e) => { e.stopPropagation(); data.onDelete?.(id); }} className="p-1.5 hover:bg-danger/20 rounded-lg text-muted hover:text-danger transition-all">
            <Trash2 size={12} />
          </button>
        </div>
      </div>
